    ...
    stages {
	    ...
	    stage('Create Function'){
					...
					steps {
							...
							sh "aws lambda create-function --function-name $env.FUNC_NAME --zip-file fileb://target/lambda-java-api-example-1.0-SNAPSHOT.jar --handler $env.HANDLER --runtime java8 --role $env.ARN"
							 }
						}  					    
		    }
		stage('Upload Function'){
					...
					steps {
							echo " The environment is ${params.Env}"
							withAWS(region:'us-west-2',credentials:'AWSLoginCred'){
							 sh 'aws lambda update-function-code  --function-name  demo-function  --zip-file fileb://target/lambda-java-api-example-1.0-SNAPSHOT.jar'
							 }
						}  					    
		    }	
